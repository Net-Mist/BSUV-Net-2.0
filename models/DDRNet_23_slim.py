...
class DAPPM(nn.Module):
    """Deep Aggregation Pyramid Pooling Module, see figure 5 in the paper
    ...
    def forward(self, x):
        ...
        width = x.shape[-1]
        height = x.shape[-2]
        x_list = []  # list of y1, y2, y3, y4, y5 in the paper notation
...
class DualResNet(nn.Module):
    def __init__(self, block, layers, num_classes=19, planes=64, spp_planes=128, head_planes=128, augment=False):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(9, planes, kernel_size=3, stride=1, padding=1),
    ...
    def forward(self, x):
        ...
        # RB 1/4 in the fig 4 of the paper
        x = self.conv1(x)
        x = self.layer1(x)
        layers.append(x)

        # first RB 1/8 in the fig 4 of the paper
